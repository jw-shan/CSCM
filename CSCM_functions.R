...
.cscm_scaleX = function(Z1,Z0,X1,X0) { #Code from Synth, credit goes to Abadie et al
  X0 <- rbind(Z0,X0)
  X1 <- rbind(Z1,X1)
  nvarsV <- dim(X0)[1]
...
holdout.csynth = function(Z1,Z0,X1,X0,Y0plot,t_int,Tmax,n_pre,lambda_min_ratio=1E-16, n_lambda=20, min_1se=FALSE) {
  
  # Find solution of SCM
  message("Estimating unit weights of SCM...")
  synth.W.full <- .quiet(Synth::synth(Z1=Z1,Z0=Z0,X1=X1,X0=X0)) #训练合成控制法
  synth.W.full <- synth.W.full$solution.w
  ...
  # customized weights
  if (n_pre>0) {
    diag(V.full)[1:n_pre+length(Z1)]=1e-8
  }
  diag(V.full) = diag(V.full)/sum(diag(V.full))
  
  message("Estimating unit weights of CSCM...")
  ...
  for (t in 1:nrow(Z0)) {
    ...
    sm <- .cscm_scaleX(Z1.holdout,Z0.holdout,t(t(X1[-(1:n_pre),])),X0[-(1:n_pre),]) #To do: add holdout functionality for X1,X0
...
countSynth = function(data,
  ...
  if (length(special.predictors)>0) {
    for (i in 1:length(special.predictors)) {
      special.predictors[[i]][[2]] = special.predictors[[i]][[2]] - (min(t_unique)-1)
    }
  }
  ...
  dataprep.full <- .quiet(Synth::dataprep(foo=df,
                                   predictors=predictors,
                                   dependent=dependent,
                                   special.predictors=special.predictors,
                                   unit.variable=unit.variable,
